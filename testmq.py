...
class MQTTClient:
    ...
    async def connect2(self, clean_session=True):
        try:
            ...
            await self._send_str2(self.client_id)


            if self.lw_topic:
                await self._send_str2(self.lw_topic)
                await self._send_str2(self.lw_msg)
            if self.user is not None:
                await self._send_str2(self.user)
                await self._send_str2(self.pswd)
    ...
    async def publish2(self, topic, msg, retain=False, qos=0):
        try:
            print("Publishing...")
